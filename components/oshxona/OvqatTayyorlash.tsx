...
export const OvqatTayyorlash: React.FC = () => {
  ...
  const handleConfirmProduction = async () => {
    if (!selectedTovar || inputQty <= 0 || currentRecipe.length === 0) {
      showNotification("Tayyorlanadigan taom va miqdorni kiriting!", "warning");
      return;
    }
    ...
    for (const ing of currentRecipe) {
      ...
      if (!fifoRes.isPossible) {
        showNotification(`Xatolik! Omboringizda ${ing.productName} yetarli emas. Faktik sarf: ${actualNeeded} ${ing.unit}. Kamchilik: ${fifoRes.shortage.toFixed(3)}`, "error");
        anyShortage = true;
        break;
      }
    ...
    if (res.success) {
      showNotification("Ishlab chiqarish yakunlandi. Taom omborga 'Ovqat' turi bilan kirim qilindi.", "success");
      setInputQty(0);
      setSelectedTovar(null);
      loadData();
    } else {
      showNotification("Xatolik! Ma'lumotlar saqlanmadi.", "error");
    }
    setSaveLoading(false);
  };
