...
export const NormaManager: React.FC<NormaManagerProps> = ({ globalFilter, userRole }) => {
  ...
  const handleSave = async () => {
    if (!selectedTovar || ingredients.length === 0) { 
      showNotification("Xatolik! Tovar va masalliqlar tanlanishi shart.", "warning"); 
      return; 
    }
  ...
  const displayIds = normaIds;

  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      <div className="flex justify-between items-end">
        <div>
          <h3 className="text-2xl font-black text-white tracking-tighter uppercase italic">Retseptlar <span className="text-orange-500">Arvixi</span></h3>
          <p className="text-zinc-500 font-bold text-[9px] uppercase tracking-[0.3em] mt-1">Ishlab chiqarish normalari boshqaruvi</p>
        </div>
        {canAdd && (
          <button onClick={() => setView('add')} className="w-16 h-16 bg-orange-600 text-white rounded-2xl flex items-center justify-center text-2xl shadow-xl hover:scale-105 active:scale-95 transition-all">
            +
          </button>
        )}
      </div>
            ...
            {displayIds.length > 0 ? displayIds.map((nId, idx) => {
              ...
              return (
                <tr key={idx} className="group">
                  <td className="text-zinc-600 font-bold text-[10px]">#{nId.toString().slice(-6)}</td>
                  <td className="font-bold text-white uppercase italic group-hover:text-orange-400 transition-colors">
                    {f['tovar']}
                  </td>
                  <td>
                    <div className="flex items-center gap-2 text-[10px] font-black">
                      <span className="text-zinc-400">{f['tovar miqdor']} {f['ikkilamchi birlik']}</span>
                      <span className="text-orange-500/50">→</span>
                      <span className="text-orange-400">{f['asosiy miqdor']} {f['tovar birlik']}</span>
                    </div>
                  </td>
                  <td>
                    <div className="flex flex-wrap gap-1.5">
                      {items.map((it, i) => (
                        <span key={i} className="px-2 py-0.5 bg-zinc-900 rounded text-[9px] font-bold text-zinc-500 border border-white/5">
                          {it['product']}: {it['product miqdor']} {it['product birlik']}
                        </span>
                      ))}
                    </div>
                  </td>
                  <td className="text-zinc-600 font-medium text-[10px]">
                    {f['norma rasxod sana']?.split(' ')[0]}
                  </td>
                </tr>
              );
            }) : (
