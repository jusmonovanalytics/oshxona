...
export const GlobalFilter: React.FC<GlobalFilterProps> = ({ onFilter, onClear, currentParams }) => {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (localParams.search !== currentParams.search || 
          localParams.startDate !== currentParams.startDate || 
          localParams.endDate !== currentParams.endDate) {
        onFilter(localParams);
      }
    }, 400); // 400ms delay for performance

    return () => clearTimeout(timer);
  }, [localParams.search, localParams.startDate, localParams.endDate]);
          ...
          <div className="relative w-full max-w-md bg-[#0f172a] border border-white/10 rounded-[2.5rem] lg:rounded-[4rem] shadow-[0_0_150px_rgba(0,0,0,1)] overflow-hidden animate-in zoom-in-95 duration-500">
            ...
            <div className="p-8 lg:p-12 space-y-8 lg:space-y-10">
              ...
              <div className="pt-6 grid grid-cols-2 gap-4">
                 ...
                 <button onClick={() => setIsOpen(false)} className="py-5 bg-indigo-600 text-white rounded-[1.5rem] lg:rounded-[2rem] text-[9px] lg:text-[10px] font-black uppercase tracking-widest">Yopish</button>
              </div>
            </div>
          </div>
        </div>
