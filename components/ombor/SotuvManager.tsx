...
export const SotuvManager: React.FC<SotuvManagerProps> = ({ globalFilter }) => {
  ...
  const handleSale = async () => {
    if (!selectedTovar || aslSotuvMiqdori <= 0) { showNotification("Iltimos, miqdorni kiriting!", "warning"); return; }

    const batches = getAvailableTovarBatches(selectedTovar['tovar id']);
    const totalAvailableAsl = batches.reduce((s, b) => s + b.remainingAsl, 0);

    if (aslSotuvMiqdori > totalAvailableAsl) {
      showNotification(`Omborda yetarli tovar yo'q! Mavjud: ${totalAvailableAsl.toFixed(2)} ${selectedTovar['tovar asl birlik'] || 'porsiya'}`, "error");
      return;
    }
    ...
    if (res.success) {
      showNotification("Sotuv muvaffaqiyatli amalga oshirildi!", "success");
